...
export default defineType({
	...
	fields: [
		...
		defineField({
			...
			description:
				"Main image for image posts or cover image for other content",
		}),
		defineField({
			name: "tags",
			title: "Tags",
		...
		defineField({
			...
			type: "number",
			description:
				"Order for featured playground items (lower numbers appear first)",
	...
	preview: {
		...
		prepare(selection) {
			...
			return {
				...
				subtitle: `${typeLabel} • ${date}${orderInfo}${tagCount}${
					featured ? " • Featured" : ""
				}`,
				media,
			};
